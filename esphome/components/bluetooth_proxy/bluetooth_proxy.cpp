...
void BluetoothProxy::send_api_packet_(const esp32_ble_tracker::ESPBTDevice &device) {
  ...
  for (auto uuid : device.get_service_uuids()) {
    resp.service_uuids.push_back(std::move(uuid.to_string()));
  }
...
void BluetoothProxy::loop() {
  ...
  if (this->send_service_ == this->services_.size()) {
    ...
    ESP_LOGD(TAG, "Start send: Free MALLOC_CAP_8BIT Size: %u bytes", heap_caps_get_free_size(MALLOC_CAP_8BIT));
    ...
    ESP_LOGD(TAG, "End send: Free MALLOC_CAP_8BIT Size: %u bytes", heap_caps_get_free_size(MALLOC_CAP_8BIT));
    heap_caps_check_integrity_all(true);
...
void BluetoothProxy::bluetooth_gatt_write(const api::BluetoothGATTWriteRequest &msg) {
  ...
  ESP_LOGV(TAG, "Writing GATT characteristic %s (response=%i)", characteristic->uuid.to_string().c_str(), msg.response);
...
void BluetoothProxy::bluetooth_gatt_write_descriptor(const api::BluetoothGATTWriteDescriptorRequest &msg) {
  ...
  ESP_LOGV(TAG, "Writing GATT characteristic descriptor %s -> %s",
